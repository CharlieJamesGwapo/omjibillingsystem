...
export default function Staff() {
  ...
  const fetchUsers = async () => {
    try {
      ...
      let admins: User[] = [], techs: User[] = [];
      let adminTotal = 0, techTotal = 0;

      if (roleFilter !== 'technician') {
        const r = await api.get<{ data: User[]; total: number }>(`/users?role=admin&limit=100${searchParam}`);
        admins = r.data.data ?? [];
        adminTotal = r.data.total ?? 0;
      }
      if (roleFilter !== 'admin') {
        const r = await api.get<{ data: User[]; total: number }>(`/users?role=technician&limit=100${searchParam}`);
        techs = r.data.data ?? [];
        techTotal = r.data.total ?? 0;
      }

      const combined = [...admins, ...techs];
      const start = (page - 1) * LIMIT;
      setUsers(combined.slice(start, start + LIMIT));
      setTotal(adminTotal + techTotal);
    } catch (err: any) {
  ...
  const handleDelete = async (id: string) => {
    try {
      ...
      const msg = err?.response?.data?.error || err?.message || 'Failed to delete staff member';
      setError(msg);
      toast.error(msg);
    }
  };
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <div className="h-4 w-48 rounded-lg bg-white/5 animate-pulse mt-2" />
          </div>
          <div className="h-10 w-28 rounded-lg bg-white/5 animate-pulse" />
        </div>
        <div className="glass-card p-0 overflow-hidden">
          {[...Array(5)].map((_, i) => (
            <div key={i} className="h-14 border-b border-white/[0.03] px-4 flex items-center gap-4">
              <div className="w-8 h-8 rounded-full bg-white/5 animate-pulse" />
              <div className="flex-1">
                <div className="h-3 w-32 rounded bg-white/5 animate-pulse mb-1.5" />
  ...
  return (
    <div className="space-y-6 animate-in">
      ...
      <div className="flex flex-col sm:flex-row gap-3 animate-in animate-in-1">
            ...
            <div key={user.id} className="glass-card p-4">
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="font-mono text-xs text-text-secondary mt-0.5">{user.phone}</p>
                  {user.email && <p className="text-xs text-text-secondary/70 mt-0.5 truncate">{user.email}</p>}
